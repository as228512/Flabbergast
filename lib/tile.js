...
export const handleTileClick = e => {
  ...
  if (isStartOfWord) {
    ...
    isValidWord(getCurrentWordField().innerHTML);
  }
};
...
export const submitWord = (word, wasValid) => {
  if (wasValid) awardPoints(word);
  resetCurrentWordField();
};

const toggleWordSelection = (isStartOfWord, currentTile) => {
  if (isStartOfWord) {
