...
export default class Game {
  ...
  toggleHighScoreModel() {
    ...
    if (model) {
      ...
      submitButton.onclick = e => {
        const respObj = gameUtil.isValidName(nameField.value);
        if (!respObj.resp) {
          e.preventDefault();

          if (respObj.err === "inValidName")
            nameField.value = "Enter name here...";
          else nameField.value = "Max length exceeded (10)";
  ...
  isHighScore() {
    ...
    const playerScore = this.board.score;
